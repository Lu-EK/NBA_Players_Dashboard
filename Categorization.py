...
def offensive_profile(player_stats, player_stats_ranked) -> str:
    ...
    ## Creator/Facilitator
    if (player_stats['AST%'] >= 25.0) and (player_stats['USG%'] >= 20.0) and (player_stats['AST/TOV'] >= 1.5):
        return 'Creator/Facilitator'

    ## Pure Shooter/Stretcher:
    elif (player_stats['3P%'] > 0.37) and (player_stats['3PAr'] > 0.5) and (player_stats_ranked['eFG% ranked'] <= 4) and (player_stats['USG%'] <= 20.0):
        return 'Pure Shooter/Stretcher'

    # Paint Threat
    elif (player_stats['%FGA 0-3'] > 0.5) and (player_stats_ranked['ORB% ranked'] <= 3) and (player_stats_ranked['eFG% ranked'] <= 3):
        return 'Paint Threat'

    ## Slasher
    elif (player_stats['%FGA 0-3']) > 0.2 and (player_stats['%FGA 3-10'] + player_stats['%FGA 10-16']) > 0.2 and (player_stats_ranked['FTr ranked'] <= 4) and (player_stats['%FGA 3P'] < 20.0):
        return 'Slasher'

    ## Versatile Scorer 
    elif (player_stats_ranked['std_areas_FGA ranked'] >= 5) and (player_stats['USG%'] >= 20.0) and (player_stats_ranked['FG ranked'] <= 5):
...
def defensive_profile(player_stats, player_stats_ranked) -> str:
    ...
    ## Paint Protector/Rim Defender
    if player_stats['DRB%'] >= 25.0 and player_stats_ranked['BLK% ranked'] <= 3 and player_stats_ranked['DBPM'] <= 3:
    ...
    ## Switchable Defender
    elif player_stats['DRB%'] >= 20.0 and player_stats_ranked['BLK% ranked'] <= 6 and player_stats_ranked['DBPM ranked'] <= 4 and player_stats_ranked['DWS ranked'] <= 4:
        return 'Switchable Defender'

    ## Rebounding Specialist
    elif player_stats['TRB%'] >= 10.0 and player_stats_ranked['DRB% ranked'] <= 2 and player_stats['DRB%'] >= 20.0:
