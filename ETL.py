...
import os
import subprocess
from Categorization import offensive_profile, defensive_profile
START_YEAR = 2020
END_YEAR = 2024
...
def transform_data(regular_dataset, advanced_dataset, shooting_splits):
    ...
    advanced_dataset = advanced_dataset[~advanced_dataset['Rk'].duplicated(keep='first')]
    ...
    full_dataset_raw = pd.concat([regular_dataset, advanced_dataset[columns_to_concat]], axis=1)
    ...
    full_dataset = pd.merge(full_dataset, shooting_splits, on='Player', how='left').reset_index()
    
    
    stats_to_keep = ['Pos_x', '3P%_x', 'Tm_x']

    cols_to_drop = [col for col in full_dataset if col not in stats_to_keep and isinstance(col, str) and (col.endswith('_x') or col.endswith('_y'))]
    ...
    full_dataset.rename(columns={'3P%_x' : '3P%', 'Tm_x' : 'Team', 'Dist.' : 'Average distance (ft.) of FGA', '-9999' : 'player_code', '0-3' : '%FGA 0-3', '3-10' : '%FGA 3-10', '10-16' : '%FGA 10-16', '16-3P' : '%FGA 16-3P', 'Pos_x' : 'Pos'}, inplace=True)
    full_dataset.drop(columns=['#', '%FGA', '%3PA', 'Att.', 'player_code'],inplace=True)
    full_dataset = full_dataset.loc[:, ~full_dataset.columns.duplicated()]
    full_dataset.iloc[:, -4:] = full_dataset.iloc[:, -4:].apply(lambda x : x.astype(float) * 100)
    ...
    full_dataset['std_areas_FGA'] = fga_area_columns.apply(lambda row:np.std(row), axis=1)
    ...
    for column in columns_to_rank:
        ...
        full_dataset_ranked.insert(full_dataset_ranked.columns.get_loc(column) + 1, new_column_name, range_numbers)
    ...
    for index, row in full_dataset.iterrows():
        ...
        full_dataset.at[index, 'Offensive Profile'] = offensive_profile(row, full_dataset_ranked.loc[index])
        # Assign defensive profile to a player    
        full_dataset.at[index, 'Defensive Profile'] = defensive_profile(row, full_dataset_ranked.loc[index])

    return full_dataset, full_dataset_ranked
    
def export_data_to_csv(start_year, full_dataset, full_dataset_ranked):
    full_dataset.to_csv(f'/home/lucas/Data Science/Project NBA/datasets/combined/regular_dataset_{start_year}_{start_year + 1}.csv')
    full_dataset_ranked.to_csv(f'/home/lucas/Data Science/Project NBA/datasets/combined/ranked_dataset_{start_year}_{start_year + 1}.csv')
...
for year in range(START_YEAR, END_YEAR):
   ...
   # globals()[f'advanced_dataset_{year}_{year+1}'] = advanced_df
    ...
    full_dataset, full_dataset_ranked = transform_data(regular_df, advanced_df, shooting_splits_df)
